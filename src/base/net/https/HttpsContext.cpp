...
uvloop::HttpsContext::HttpsContext(TlsContext *tls, const std::weak_ptr<IHttpListener> &listener) :
...
uvloop::HttpsContext::~HttpsContext() = default;


void uvloop::HttpsContext::append(char *data, size_t size)
{
    if (m_mode == TLS_AUTO) {
...
bool uvloop::HttpsContext::write(BIO *bio)
{
...
void uvloop::HttpsContext::parse(char *data, size_t size)
{
...
void uvloop::HttpsContext::shutdown()
{
    close();
}


void uvloop::HttpsContext::write(std::string &&data, bool close)
{
    m_close = close;
