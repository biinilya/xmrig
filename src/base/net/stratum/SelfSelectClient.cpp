...
uvloop::SelfSelectClient::SelfSelectClient(int id, const char *agent, IClientListener *listener, bool submitToOrigin) :
...
uvloop::SelfSelectClient::~SelfSelectClient()
{
    delete m_client;
}


int64_t uvloop::SelfSelectClient::submit(const JobResult &result)
{
    if (m_submitToOrigin) {
...
void uvloop::SelfSelectClient::tick(uint64_t now)
{
    m_client->tick(now);
...
void uvloop::SelfSelectClient::onJobReceived(IClient *, const Job &job, const rapidjson::Value &)
{
    m_job = job;
...
void uvloop::SelfSelectClient::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
bool uvloop::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
void uvloop::SelfSelectClient::getBlockTemplate()
{
    setState(WaitState);
...
void uvloop::SelfSelectClient::retry()
{
    setState(RetryState);
}


void uvloop::SelfSelectClient::setState(State state)
{
    if (m_state == state) {
        return;
    }
...
void uvloop::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    using namespace rapidjson;
    Document doc(kObjectType);
    auto &allocator = doc.GetAllocator();
...
void uvloop::SelfSelectClient::submitOriginDaemon(const JobResult& result)
{
...
void uvloop::SelfSelectClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
