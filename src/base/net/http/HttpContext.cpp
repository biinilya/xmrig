...
uvloop::HttpContext::HttpContext(int parser_type, const std::weak_ptr<IHttpListener> &listener) :
    HttpData(SEQUENCE++),
...
uvloop::HttpContext::~HttpContext()
{
    delete m_tcp;
    delete m_parser;
}
...
void uvloop::HttpContext::write(std::string &&data, bool close)
{
...
bool uvloop::HttpContext::isRequest() const
{
    return m_parser->type == HTTP_REQUEST;
}


bool uvloop::HttpContext::parse(const char *data, size_t size)
{
    if (size == 0) {
        return true;
    }
...
std::string uvloop::HttpContext::ip() const
{
    char ip[46]           = {};
...
uint64_t uvloop::HttpContext::elapsed() const
{
    return Chrono::steadyMSecs() - m_timestamp;
}


void uvloop::HttpContext::close(int status)
{
    if (!get(id())) {
        return;
    }
...
uvloop::HttpContext *uvloop::HttpContext::get(uint64_t id)
{
...
void uvloop::HttpContext::closeAll()
{
    for (auto &kv : storage) {
...
int uvloop::HttpContext::onHeaderField(llhttp_t *parser, const char *at, size_t length)
{
...
int uvloop::HttpContext::onHeaderValue(llhttp_t *parser, const char *at, size_t length)
{
    auto ctx = static_cast<HttpContext*>(parser->data);
...
void uvloop::HttpContext::attach(llhttp_settings_t *settings)
{
...
void uvloop::HttpContext::setHeader()
{
