...
void uvloop::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void uvloop::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void uvloop::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void uvloop::JobResults::submit(const Job& job, uint32_t nonce, const uint8_t* result, const uint8_t* miner_signature)
{
    submit(JobResult(job, nonce, result, nullptr, nullptr, miner_signature));
}


void uvloop::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(XMRIG_FEATURE_OPENCL) || defined(XMRIG_FEATURE_CUDA)
void uvloop::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
