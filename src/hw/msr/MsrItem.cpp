...
uvloop::MsrItem::MsrItem(const rapidjson::Value &value)
{
    if (!value.IsString()) {
        return;
    }
...
rapidjson::Value uvloop::MsrItem::toJSON(rapidjson::Document &doc) const
{
    return toString().toJSON(doc);
}


uvloop::String uvloop::MsrItem::toString() const
{
