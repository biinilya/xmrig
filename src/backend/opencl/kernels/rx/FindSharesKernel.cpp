...
void uvloop::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void uvloop::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void uvloop::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void uvloop::FindSharesKernel::setNonce(uint32_t nonce)
{
